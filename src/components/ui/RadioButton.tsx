...
export function RadioButton({ label, inputName }: RadioButton) {
  return (
    <label htmlFor={label} className="relative cursor-pointer flex items-center group">
      <input type="radio" name={inputName} id={label} className="hidden group-[input]:" />
      <span className={`label relative block float-left mr-[10px] w-[20px] h-[20px]
      ...
      group-hover:after:scale-[3.6]`} />{label}
    </label>
  )
}