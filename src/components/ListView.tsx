    ...
    <div className="scrollbar md:grid-cols-2 grid max-h-[750px] grid-cols-1 gap-12 overflow-y-scroll">
      <section className=" grid gap-y-12">
        {products.map((product) => {
          ...
          return (
            <article key={id} className="flex flex-col laptop:gap-12 gap-6 px-4 tablet:flex-row tablet:justify-center pt-4 ">
              <Image
              ...
              <div className="font-primary text-white relative">
                ...
                <Link href={`/products/${id}`} className=" bg-cta px-2 py-1 text-xs absolute right-0 tablet:static">
                  Details
                </Link>
              </div>
            </article>
